...
def build(path: str) -> JudgeResult:
    ...
    config_command = "cmake -B ./build" + (
        ' -G "MinGW Makefiles"' if os.name == "nt" else ""
    )
    ...
    output += build_r.stdout.decode(errors="ignore") + build_r.stderr.decode(
        errors="ignore"
    )
...
def run_exe(
    path: str, args: List[str], rediect_input: Union[None, str]
    ...
    if rediect_input is None:
        ...
        proc = subprocess.Popen(
            args, stdin=file, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def test(path: str, case: Union[Case, MalformedCase]) -> JudgeResult:
    ...
    if not os.path.exists(exe_path):
        return JudgeResult(
            "pretest", False, "Output executable file docman does not exist."
        )
    if isinstance(case, MalformedCase):
        _, code, log, timeout = run_exe(
